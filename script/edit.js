...
function addInput(editInfo) {
  if (editInfo !== undefined && editInfo !== "") {
    idInput.value = editInfo[0];
    nameInput.value = editInfo[1];
    ageInput.value = editInfo[2];
    weightInput.value = editInfo[4];
    lengthInput.value = editInfo[5];
    colorInput.value = editInfo[7];
    typeInput.value = editInfo[3];
    changeBreed(); // đưa thông tin về breed phù hợp
    breedInput.value = editInfo[6];
    vaccinatedInput.checked = editInfo[8];
    dewormedInput.checked = editInfo[9];
    sterilizedInput.checked = editInfo[10];
  }
  removePet(petArr, editInfo[editInfo.length - 1]);
  // Xóa thông tin pet
}
